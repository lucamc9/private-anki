...
def load_db(course):
    file_name = 'databases/' + course + '-db.csv'
    db = pd.read_csv(file_name, sep='\t')
    return db

def load_stats(course):
    file_name = 'statistics/' + course + '-stats.csv'
    stats = pd.read_csv(file_name, sep='\t')
    return stats
...
def display_stats(stats):
    ...
    print('Latest score: {} {}/{} at {}\n'.format(date_latest, correct_latest, answered_latest, topic_latest))
...
def save_stats(course, wrong_qs, all_qs, topic):
    ...
    try:
        stats_db = pd.read_csv('statistics/' + course + '-stats.csv', sep='\t')
        stats_db = stats_db.append(stats_df)
        stats_db.to_csv('statistics/' + course + '-stats.csv', sep='\t', index=False)
    except:
        stats_df.to_csv('statistics/' + course + '-stats.csv', sep='\t', index=False)
